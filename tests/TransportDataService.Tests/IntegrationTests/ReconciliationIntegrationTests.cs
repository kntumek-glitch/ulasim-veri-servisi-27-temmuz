...
public class ReconciliationIntegrationTests
{
    ...
    private async Task SeedReconciliationDataAsync()
    {
        ...
        var run = new GtfsImportRun { Status = "Completed", IsActive = true, FinishedAt = DateTime.UtcNow };
        _context.GtfsImportRuns.Add(run);

        // Stops table (CSV import data)
        var stops = new List<Stop>
        {
            new() { ExternalStopId = "S1", Name = "Stop 1", Latitude = 38.4, Longitude = 27.1 },
            new() { ExternalStopId = "S2", Name = "Stop 2", Latitude = 38.5, Longitude = 27.2 },
            new() { ExternalStopId = "S3", Name = "Stop 3", Latitude = 38.6, Longitude = 27.3 },
            new() { ExternalStopId = "S4", Name = "Stop 4", Latitude = 38.7, Longitude = 27.4 },
            new() { ExternalStopId = "S5", Name = "Stop 5", Latitude = 38.8, Longitude = 27.5 },
            new() { ExternalStopId = "S6", Name = "Stop 6", Latitude = 38.9, Longitude = 27.6 },
        };

        // GTFS Stops (from GTFS import)
        var gtfsStops = new List<GtfsStop>
        {
            new() { GtfsImportRun = run, StopId = "S1", StopName = "Stop 1", StopLat = 38.4, StopLon = 27.1, StopCode = "S1" },      // Direct match
            new() { GtfsImportRun = run, StopId = "S2", StopName = "Stop 2", StopLat = 38.5, StopLon = 27.2, StopCode = "S2" },      // Direct match
            new() { GtfsImportRun = run, StopId = "S3", StopName = "Stop 3 Modified", StopLat = 38.6, StopLon = 27.3, StopCode = "S3" }, // Name mismatch
            new() { GtfsImportRun = run, StopId = "S4", StopName = "Stop 4", StopLat = 38.7002, StopLon = 27.4002, StopCode = "S4" }, // Coordinate mismatch
            new() { GtfsImportRun = run, StopId = "S7", StopName = "Stop 7", StopLat = 38.9, StopLon = 27.7, StopCode = "S7" },      // Missing in Stops
            new() { GtfsImportRun = run, StopId = "S8", StopName = "Stop 8", StopLat = 39.0, StopLon = 27.8, StopCode = "S8" },      // Missing in Stops
        };
