...
public class RoutingController : ControllerBase
{
    ...
    public async Task<IActionResult> GetWalkingRoute([FromBody] WalkRoutingRequestDto request, CancellationToken cancellationToken)
    {
        ...
        {
            response.Alternatives = new System.Collections.Generic.List<WalkRoutingResponseDto>();
            foreach (var alt in result.Alternatives)
            {
                var altDto = new WalkRoutingResponseDto
                {
                ...
                if (request.IncludeGeometry)
                {
                    altDto.Geometry = alt.GeometryGeoJson;
                }
                response.Alternatives.Add(altDto);
            }
        }
    ...
    /// <summary>
    /// Calculates a driving routing path between two coordinates.
    /// </summary>
    [HttpPost("drive")]
    public async Task<IActionResult> GetDrivingRoute([FromBody] WalkRoutingRequestDto request, CancellationToken cancellationToken)
    {
        if (request == null)
            return BadRequest(new ProblemDetails { Title = "Geçersiz İstek", Detail = "İstek gövdesi boş olamaz.", Status = 400 });
